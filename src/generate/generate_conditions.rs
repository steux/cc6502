...
impl<'a, 'b> GeneratorState<'a> {
    pub(crate) fn generate_ternary(&mut self, condition: &Expr, alternatives: &Expr, pos: usize) -> Result<ExprType, Error>
    {
        match alternatives {
    ...
    pub(crate) fn generate_expr_cond(&mut self, expr: &Expr, pos: usize) -> Result<ExprType, Error>
    {
        if self.acc_in_use {
            self.sasm(PHA)?; 
            if self.tmp_in_use {
    ...
    pub(crate) fn generate_if(&mut self, condition: &'a Expr, body: &'a StatementLoc<'a>, else_body: Option<&'a StatementLoc<'a>>, pos: usize) -> Result<(), Error>
    {
        self.local_label_counter_if += 1;
        let ifend_label = format!(".ifend{}", self.local_label_counter_if);
        match else_body {
    ...
    pub(crate) fn generate_switch(&mut self, expr: &'a Expr, cases: &'a Vec<(Vec<i32>, Vec<StatementLoc<'a>>)>, pos: usize) -> Result<(), Error>
    {
