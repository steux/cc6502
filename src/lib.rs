...
// DONE: Bad state flag state after a cmp
// DONE: Bad state flag after a JSR
// TODO: Bug if {char i;}{char i;}
...
void main() { fn2(); fn3(); }
    ...
    #[test]
    fn shift_16bits_test5() {
        let args = sargs(1);
        let input = "char *ptr; unsigned int s; void main() { s |= ptr[Y++] << 8; }";
        let mut output = Vec::new();
        compile(input.as_bytes(), &mut output, &args, simple_build).unwrap();
        let result = str::from_utf8(&output).unwrap();
        print!("{:?}", result);
        assert!(result.contains("LDA (ptr),Y\n\tORA s+1\n\tSTA s+1\n\tINY"));
    }
    ...
    #[test]
    fn const_ptr_arithm_test() {
        let args = sargs(1);
        let input = "const char *ptr = \"\"; void main() { char x = (ptr >> 8) + 10; }";
        let mut output = Vec::new();
        compile(input.as_bytes(), &mut output, &args, simple_build).unwrap();
        let result = str::from_utf8(&output).unwrap();
        print!("{:?}", result);
